...
function nextPage(){
	...
	//}
	var data = "type="+'${type}'+"&page="+p+"&flag=1"+"&pic_status="+pic_status;
	var loadi = layer.load(0);
	$.ajax({
		  type: 'POST',
		  async:false,
		  url: "web/search_typeSearch",
		  data: data,
		  dataType: 'html',
...
<div style=" width:980px; margin:0 auto;">
    <div class="_body_top_menu">
    	<div style="background-image:url(<%=basePath%>module/web/images/list_icon1_03.png); background-position:0 center; background-repeat:no-repeat; padding-left:32px; height:32px; position:absolute; left:15px; top:12px;" ><span style="font-size:18px; line-height:42px;">${skey}<!-- (${limit}条) --></span></div>
        <div id="typeDivs" style="padding-left: 50px;">
        <s:if test="!typeDatas.isEmpty()">
		<s:iterator var="s" value="typeDatas" status="status">
        <div id="${s.code }" class="_menu_btn" onclick="redirectList('search_typeSearch?type=${s.code}');"><span id="d_show_a">${s.fullname }</span></div>
        </s:iterator>
        </s:if>
