...
<script type="text/javascript">
	...
	$(function(){
		...
		$.post("touch_main_showGouWuDetail", parms, function(json) {
			var code = json.code;
			var hint = json.hint;
			dialog(hint);
			if (code == 1) {
				...
				$("#toAddress").click(function(){ window.location.href="<%=curl%>nav.jsp?flag=1&to_id="+o.fuid+"&category=002015&tab=0"; });
				...
				if(more.length > 0){
					...
					for(i=0;i<more.length;i++){
						var url = '<%=curl%>nav.jsp?flag=1&to_id='+more[i].fuid+'&category=002015&tab=0';
						html += '<li><div class="left"><i class="fa fa-map-marker"></i></div><div class="text"><h3>'+more[i].fullname+'</h3><p>'+more[i].address+'<br />电话：'+more[i].phone+'</p></div><div class="right"><a class="btn btn-lg btn-yule" onclick=window.location.href="<%=curl%>nav_gouwu_detail.jsp?id='+more[i].fuid+'"><i class="fa fa-align-justify"></i>详情</a><a class="btn btn-lg btn-gouwu" href="'+url+'"><i class="fa fa-level-up"></i>前往</a></div></li>';
					}
					$("#more_list").html(html);
				}else{
					$("#more_list").html('<span style="font-size: 20px; display: block;padding: 30px 0; text-align: center;">没有更多推荐的购物商铺</span>');
				}
			}
		}, "json");
	});
		...
		<div class="col-md-12 col-sm-12">
			<div class="list-1">
				<ul class="tabs" id="menu">
					...
					<li><a href="more_list" data-toggle="tab">更多购物</a></li>
				</ul>
