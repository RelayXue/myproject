...
<head>
 ...
 jQuery(document).ready(function() {
	...
	$("#page").AutoPage({
			...
			url:'buNews?type=<%=type%>&skey=${skey}',
			showNum : 6 , // 显示数字
			show:1
		});
		//--------------全选-------------
 ...
 function serach(){
	 ...
	 var url="buNews?type="+'${type}'+"&skey="+skey;
...
<body>
	...
	<div class="rhandle">
				...
				<s:iterator value="buNews_list" var="list" status="stuts">
					<tr>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buNews!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buNews!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="buNews!delete?type=${type}" method="post">
