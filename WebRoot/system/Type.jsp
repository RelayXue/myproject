	...
	<head>
	 ...
	 jQuery(document).ready(function($) {
		 $("#page").AutoPage({
					...
					url:'Datadictionary?code=${code}&skey=${skey}',
					showNum : 6  // 显示数字
					,show:1
				});
	 });
	 function serach(){
	 var skey=$("#mc").val();
	 var url="Datadictionary!show?skey="+skey;
	 window.location.href=url;
	 }
	 function add(){
	 var url="<%=basePath %>system/TypeEditor.jsp";
	 window.location.href=url;
	 }
	</script>
		<style>
				...
				<table width="100%" cellSpacing="0" cellPadding="0" class="table">
					...
					<s:else>
						...
						<s:if test="com.hisUpdate==true">
							<a class="blue-12"  href="<%=basePath %>Datadictionary!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}">修改</a>
						</s:if>
						<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('确定删除！')){return false;}" class="blue-12"
								href="Datadictionary!delete?id=<s:property value="#list.fuid"  />&skey=${skey}">删除</a>
						</s:if>
							<a href="Datadictionary!showChild2?code=<s:property value="#list.code"  />" class="blue-12" >编辑子目录</a>
