...
<head>
 ...
 function serach(){
	 ...
	 var url="buLink?skey="+skey;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath%>/business/BuLinkEditor.jsp";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
	...
	<div class="rhandle">
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buLink_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buLink!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buLink!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="buLink!delete?indexPage=${indexPage}" method="post">
