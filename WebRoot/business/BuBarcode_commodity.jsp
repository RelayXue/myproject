...
<head>
	 ...
	 jQuery(document).ready(function($) {
		 //--------------分页-------------
			$("#page").AutoPage({
					...
					url:'buCommodity!BcodeShow?skey=${skey}',
					showNum : 6  // 显示数字
					,show:1
				});
				//--------------全选-------------
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buCommodity!BcodeShow?skey="+skey;
 ...
 function Generate(){
	 	 ...
	 	 if(val.length==0){
	 	 	...
	 	 	var names="";
	 	 	 ...
	 	 	 $("#form1").attr("action","buCommodity!generate?skey="+skey);
	 	 	 $("#form1").submit();
	 	 }
	 	}
	 	//审核
	 	function examine(fuid){
	 		$("#ifr").attr("src","<%=basePath %>buCommodity!examineShow?id="+fuid+"&time="+new Date());
			 	 $('#dd').dialog({
				    modal:true,
					title:"审核列表"
				  });
			...
			<table width="100%" border="0">
				...
				<s:iterator value="buCommodity_list" var="list" status="stuts">
					<tr>
						...
						<td>
						<s:if test="com.hisOther.get('business_sh_sh')==true">
							<s:if test="#list.status!=1">
