...
<head>
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buAffairs?skey="+skey;
		 window.location.href=url;
	 }
	  //---添加---
	 function add(){
	 var url="<%=basePath %>business/BuAffairsEditor.jsp";
	 window.location.href=url;
	 }
	...
	<div class="rhandle">
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buAffairs_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							<s:property value="#list.reported" />&nbsp;
						 </td>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buAffairs!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buAffairs!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-trash icon-white"></i>删除</a>
							</s:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div style="display: none;">
			<form id="mform" action="buAffairs!delete" method="post">
