...
jQuery(document).ready(function($) {
 //--------------分页-------------
	$("#page").AutoPage({
			...
			url:'buDining!generate?skey=${skey}',
			showNum : 6  // 显示数字
			,show:1
		});
		//--------------全选-------------
...
//---搜索----	  
 function serach(){
	 var skey=$("#mc").val();
	 var url="buDining!generate?skey="+skey;
...
function Generate(fuid,name){
	 ...
	 if(fuid!=null&&fuid.length>0){
		 $("#ids").attr("value",fuid);
	 	 ...
	 	 $("#form1").attr("action","buDining!generate?skey="+skey);
	 	 $("#form1").submit();
	 }else{
		 if(val.length==0){
			 	alert("请先选择!");
...
<body>
	<div class="rightnav">
		...
		<div class="rhandle">
				...
				<a onclick="if(!confirm('确定生成所有！生成所有需要些时间，请耐心等待！')){return false;}"  class="btn btn-success" href="<%=basePath %>buDining!generateAll"><i class="icon-plus-sign icon-white"></i>生成所有</a>
		</div>
		...
		<div style="display: none;">
			<form id="form1" action=" buDining!delete" method="post">
		...
		<div style="display: none;">
		 	 <div id="dd" title="查看美食" data-options="iconCls:'icon-search'" style="width: 850px;height:550px;top: 37px;overflow:hidden;">
