...
<head>
 ...
 function serach(){
	 ...
	 var url="buAttractions?type="+'${type}'+"&skey="+skey;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath%>business/BuAttractionsEditor.jsp?type=${type}";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
...
<body>
	<div class="rightnav">
			<a href="#" class="active">景点管理<!--<s:property value="#application.DatadictionaryMap[type]"  />--></a>
	...
	<div class="rhandle">
				...
				<s:iterator value="buAttractions_list" var="list" status="stuts">
					<tr>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buAttractions!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buAttractions!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="buAttractions!delete?type=${type}" method="post">
