...
jQuery(document).ready(function($) {
 //--------------分页-------------
	$("#page").AutoPage({
			...
			url:'buDining?skey=${skey}&typeName=${typeName}',
			showNum : 6  // 显示数字
			,show:1
		});
		//--------------全选-------------
...
//---搜索----	  
 function serach(){
	 ...
	 var url="buDining?skey="+skey+"&typeName="+typeName;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath %>business/BuDiningEditor.jsp?type="+"002018";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
...
//查看美食
function showCate(fuid){
	$("#ifmap").attr("src","<%=basePath%>buDiningSpecialty?diningid="+fuid);
		$('#dd').dialog();
...
<body>
	<div class="rightnav">
		...
		<select name="typeName" id="typeName" style="width:126px;" class="inp" >
			...
			<option value="002018004"  <s:if test="%{typeName=='002018004'}">selected</s:if>>中餐店</option>
			<option value="002018001" <s:if test="%{typeName=='002018001'}">selected</s:if>>小吃店</option>
			<option value="002018003" <s:if test="%{typeName=='002018003'}">selected</s:if>>面馆</option>
			<!--<option value="002018005" <s:if test="%{typeName=='002018005'}">selected</s:if>>快餐店</option>-->
			<!--<option value="002018002" <s:if test="%{typeName=='002018002'}">selected</s:if>>西餐店</option>-->
		</select>
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buDining_list" var="list" status="stuts">
					<tr><!--
						<td>
						...
						<td>
							<a class="btn btn-success" href="javascript:showCate('${fuid}')" ><i class="icon-share icon-white"></i>查看</a>	
							<s:if test="com.hisUpdate==true">
							<a class="btn btn-info" href="<%=basePath%>buDining!update_show?id=<s:property value="#list.fuid" />&skey=${skey}&indexPage=${indexPage}"><i class="icon-edit icon-white"></i> 编辑 </a>
							<!--<a href="<%=basePath%>buDining!update_show?id=<s:property value="#list.fuid" />&skey=${skey}&indexPage=${indexPage}">编辑</a>&nbsp;&nbsp;  -->
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buDining!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-trash icon-white"></i>删除</a>
							</s:if>
		...
		<div style="display: none;">
			<form id="mform" action=" buDining!delete" method="post">
				<input type="hidden" name="uid" value="" id="ids" />
			</form>
		</div>
		
		<div style="display: none;">
		 	 <div id="dd" title="查看美食" data-options="iconCls:'icon-search'" style="width: 850px;height:550px;top: 37px;overflow:scroll;">
