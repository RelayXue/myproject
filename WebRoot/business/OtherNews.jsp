...
<head>
 ...
 jQuery(document).ready(function() {
	...
	$("#page").AutoPage({
			...
			url:'OtherNews?type=<%=type%>&skey=${skey}',
			showNum : 6 , // 显示数字
			show:1
		});
		//--------------全选-------------
 ...
 function serach(){
	 ...
	 var url="OtherNews?type="+'${type}'+"&skey="+skey;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath%>/business/OtherNewsEditor.jsp?type=${type}";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
...
<body>
	...
	<div class="rhandle">
				...
				<s:iterator value="buNews_list" var="list" status="stuts">
					<tr>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>OtherNews!update_show_s?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>OtherNews!delete_s?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="OtherNews!delete_s?type=${type}" method="post">
