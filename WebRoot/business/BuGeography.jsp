...
<head>
 ...
 jQuery(document).ready(function() {
	...
	$("#page").AutoPage({
			...
			url:'buGeography?type=<%=type%>&skey=${skey}',
			showNum : 6 , // 显示数字
			show:1
		});
		//--------------全选-------------
 ...
 function serach(){
	 var skey=$("#mc").val();
	//var url = "buGeography?skey="+skey;
	 var url="buGeography?type="+'${type}'+"&skey="+skey;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath %>business/BuGeographyEditor.jsp?type=<%=type.substring(0,6)%>";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
	...
	<div class="rhandle">
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buGeography_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buGeography!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buGeography!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="buGeography!delete?type=${type}" method="post">
