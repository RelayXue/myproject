...
jQuery(document).ready(function($) {
 //--------------分页-------------
	$("#page").AutoPage({
			...
			url:'buEntertainmentshopping!showExamine?type=<%=type%>&skey=${skey}',
			showNum : 6  // 显示数字
			,show:1
		});
		//--------------全选-------------
...
//---搜索----	
 function serach(){
	 var skey=$("#mc").val();
	 var url="buEntertainmentshopping!showExamine?type="+'${type}'+"&skey="+skey;
...
//生成二维码
function Generate(fuid,name,type){
	 var skey=$("#mc").val();
	 ...
	 if(fuid!=null&&fuid.length>0){
		 $("#ids").attr("value",fuid);
	 	 ...
	 	 $("#form1").attr("action","buEntertainmentshopping!generate?skey="+skey);
	 	 $("#form1").submit();
	 }else{
		 if(val.length==0){
			 	alert("请先选择!");
...
<body>
	<div class="rightnav">
		<a href="#" class="active">娱乐购物管理<!--<s:property value="#application.DatadictionaryMap[type]"  />--></a>
	</div>
		...
		<div class="rhandle">
			<a  onclick="if(!confirm('确定生成！')){return false;}" class="btn btn-success" href="javascript:Generate()"><i class="icon-plus-sign icon-white"></i>批量生成</a>
				<a onclick="if(!confirm('确定生成所有！生成所有需要些时间，请耐心等待！')){return false;}"  class="btn btn-success" href="<%=basePath %>buEntertainmentshopping!generateAll"><i class="icon-plus-sign icon-white"></i>生成所有</a>
		</div>
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buEntertainmentshopping_list" var="list" status="stuts">
					<tr>
						<td>
							<input name="ch" type="checkbox" types="<s:property value="#list.type"  />" sname="<s:property value="#list.fullname" />" value="<s:property value="#list.fuid"  />" />
						</td>
						<td>
						...
						<td>
							...
							<s:else>
								...
								<a onclick="if(!confirm('确定生成！')){return false;}" class="btn btn-info" href="javascript:Generate('${fuid}','<s:property value="#list.fullname" />','<s:property value="#list.type" />')"><i class="icon-share icon-white"></i>生成二维码</a>
							</s:else>
		...
		<div style="display: none;">
			<form id="form1" action="buEntertainmentshopping!delete" method="post">
				<input type="hidden" name="ids" value="" id="ids" />
				<input type="hidden" name="names" value="" id="names" />
				<input type="hidden" name="types" value="" id="types" />
			</form>
		</div>
