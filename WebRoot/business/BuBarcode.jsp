...
<head>
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buBarcode?type=<%=type%>&skey="+skey;
		 window.location.href=url;
	 }
	 function add(){
	 var url="<%=basePath %>business/BuBarcodeEditor.jsp?type=<%=type%>";
	 window.location.href=url;
	 }
	 function delMore() {
 ...
 function Generate(){
	 	 ...
	 	 if(val.length==0){
	 	 	...
	 	 	var names="";
	 	 	 ...
	 	 	 $("#form1").attr("action","buBarcode!generate?type=<%=type%>&skey=${skey}");
	 	 	 $("#form1").submit();
	 	 }
	 	}
	 	...
	 	function examine(fuid){
	 		$("#ifr").attr("src","<%=basePath %>buBarcode!examineShow?id="+fuid+"&time="+new Date());
			 	 $('#dd').dialog({
				    modal:true,
					title:"审核列表"
				  });
	...
	<div class="rightnav">
		...
		<div class="rtable">
			...
			<table width="100%" border="0">
				...
				<s:iterator value="buBarcode_list" var="list" status="stuts">
					<tr>
						<td>
							<input name="ch" type="checkbox"  sname="<s:property value="#list.Code" />" value="<s:property value="#list.fuid"  />" />
						</td>
						<td>
							<span><s:property value="#stuts.index+1" /> </span>
						</td>
							 
								<td><s:property value="#list.code" />&nbsp; </td>
						...
						<td>
						<s:if test="com.hisOther.get('business_hd_sh')==true||com.hisOther.get('business_yh_sh')==true">
							<s:if test="#list.status!=1">
						...
						<s:if test="com.hisUpdate==true">
							<a class="blue-12"  href="<%=basePath %>buBarcode!update_show?type=<%=type%>&id=<s:property value="#list.fuid"  />&skey=${skey}">修改</a>
						</s:if>
						<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('确定删除！')){return false;}" class="blue-12"
								href="<%=basePath %>buBarcode!delete?type=<%=type%>&id=<s:property value="#list.fuid"  />&skey=${skey}">删除</a>
						</s:if>
						</td>
					</tr>
				</s:iterator>
	 ...
	 <form id="mform" action="buBarcode!delete?type=<%=type%>" method="post">
