 ...
 jQuery(document).ready(function($) {
	 //--------------分页-------------
		$("#page").AutoPage({
				...
				url:'buStayRoom?skey=${skey}&stayid=${stayid}',
				showNum : 6  // 显示数字
				,show:1
			});
			//--------------全选-------------
...
function serach(){
		 var skey=$("#mc").val();
		 var url="buStayRoom?stayid="+'${stayid}'+"&skey="+skey;
		 // var url="buBasicdata?type="+'${type}'+"&skey="+skey;
		 window.location.href=url;
}
//添加
function add(){
	 var url="<%=basePath %>business/BuStayRoomEditor.jsp?stayid=${stayid}";
	 window.location.href=url;
 }
//----批量选择------
function delMore() {
...
<style>
	...
	<div class="rightnav">
		...
		<div class="rhandle">
			...
			<!-- <a href="javascript:delMore()" class="tip3">批量删除</a>-->
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buStayRoom_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							<span><s:property value="#stuts.index+1" /> </span>
								...
								<!--<td><s:property value="#list.dimages" />&nbsp; </td>-->
								<!--<td><s:property value="#list.floor" />&nbsp; </td>-->
								<!--<td><s:property value="#list.bed" />&nbsp; </td>-->
								<td><s:property value="#list.morelive" />&nbsp; </td>
								<!-- <td><s:property value="#list.internet" />&nbsp; </td>-->
								<td><s:property value="#list.price" />&nbsp; </td>
							<!--  	<td><s:property value="#list.stayid" />&nbsp; </td>-->
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buStayRoom!update_show?id=<s:property value="#list.fuid"  />&stayid=${stayid}&skey=${skey}&indexPage=${indexPage}"><i class="icon-edit icon-white"></i> 编辑 </a><!--		
							<a href="<%=basePath%>buStayRoom!update_show?id=<s:property value="#list.fuid"  />&stayid=${stayid}&skey=${skey}&indexPage=${indexPage}">编辑</a>&nbsp;&nbsp;
							-->
							</s:if>
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buStayRoom!delete?id=<s:property value="#list.fuid"  />&stayid=${stayid}&skey=${skey}&indexPage=${indexPage}"><i class="icon-trash icon-white"></i>删除</a>
							</s:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div style="display: none;">
			<form id="mform" action="buStayRoom!delete?stayid=${stayid }" method="post">
