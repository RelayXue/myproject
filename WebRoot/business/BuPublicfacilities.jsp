...
<head>
	 ...
	 jQuery(document).ready(function($) {
		 //--------------分页-------------
			$("#page").AutoPage({
					...
					url:'buPublicfacilities?skey=${skey}',
					showNum : 6  // 显示数字
					,show:1
				});
				//--------------全选-------------
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buPublicfacilities?skey="+skey;
		 window.location.href=url;
	 }
	 function add(){
	 var url="<%=basePath %>business/BuPublicfacilitiesEditor.jsp";
	 window.location.href=url;
	 }
	 function delMore() {
	...
	<div class="rightnav">
		...
		<div class="rtable">
			<table width="100%" border="0">
				...
				<s:iterator value="buPublicfacilities_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							...
							<a href="<%=basePath%>buPublicfacilities!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}">编辑</a>&nbsp;&nbsp;
							</s:if>
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('确定删除！')){return false;}" href="<%=basePath%>buPublicfacilities!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}">删除</a>
							</s:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div style="display: none;">
			<form id="mform" action="buPublicfacilities!delete" method="post">
