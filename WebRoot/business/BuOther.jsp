...
<head>
 ...
 function serach(){
	...
	//var url = "buGeography?skey="+skey;
	 var url="buGeographys?skey="+skey;
	 window.location.href=url;
 }
 //---添加---
 function add(){
	 var url="<%=basePath %>business/BuOtherEditor.jsp";
	 window.location.href=url;
 }
 //---批量选择---
 function delMore() {
...
<body>
	...
	<div class="rhandle">
				...
				<s:iterator value="buGeography_list" var="list" status="stuts">
					<tr>
						...
						<td>
							...
							<a class="btn btn-info" href="<%=basePath%>buGeographys!update_shows?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buGeographys!deletes?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-trash icon-white"></i>删除</a>
		...
		<div style="display: none;">
			<form id="mform" action="buGeographys!deletes" method="post">
