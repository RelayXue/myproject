...
<head>
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buRoad?skey="+skey;
		 window.location.href=url;
	 }
	 function add(){
	 var url="<%=basePath %>business/BuRoadEditor.jsp";
	 window.location.href=url;
	 }
	 function delMore() {
	...
	<div class="rightnav">
		...
		<div class="rtable">
			<table width="100%" border="0" >
				...
				<s:iterator value="buRoad_list" var="list" status="stuts">
					<tr>
						<td>
						...
						<td>
							...
							<!--<a href="<%=basePath%>buRoad!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}">编辑</a>-->&nbsp;&nbsp;
							<a class="btn btn-info" href="<%=basePath%>buRoad!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-edit icon-white"></i> 编辑 </a>		
							</s:if>
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('您确定删除吗？')){return false;}" class="btn btn-danger" href="<%=basePath%>buRoad!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}"><i class="icon-trash icon-white"></i>删除</a>
							<!--<a onclick="if(!confirm('确定删除！')){return false;}" href="<%=basePath%>buRoad!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}">删除</a>-->
							</s:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div style="display: none;">
			<form id="mform" action="buRoad!delete" method="post">
