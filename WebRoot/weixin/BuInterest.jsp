...
<head>
	 ...
	 function serach(){
		 var skey=$("#mc").val();
		 var url="buInterest?skey="+skey;
		 window.location.href=url;
	 }
	 function add(){
	 var url="<%=basePath %>business/BuInterestEditor.jsp?type=${type}";
	 window.location.href=url;
	 }
	 function delMore() {
	...
	<div class="rightnav">
				...
				<s:iterator value="buInterest_list" var="list" status="stuts">
					<tr>
						...
						<td>
							...
							<a href="<%=basePath%>buInterest!update_show?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}">编辑</a>&nbsp;&nbsp;
							</s:if>
							<s:if test="com.hisDelete==true">
							<a onclick="if(!confirm('确定删除！')){return false;}" href="<%=basePath%>buInterest!delete?id=<s:property value="#list.fuid"  />&skey=${skey}&indexPage=${indexPage}&type=${type}">删除</a>
							</s:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div style="display: none;">
			<form id="mform" action="buInterest!delete" method="post">
